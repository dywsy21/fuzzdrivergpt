...
class APIUsage:
    ...
    def buildAPIUsages(targetcfg, funcsig=None, skipsgcrawl=False):
        ...
        # 1. inputs collection
        if not skipsgcrawl:
            # Adjust API list gathering based on language
            if targetcfg.language == 'javascript':
                analyzer.analyze_wrap({'anamode': 'listapisfromjs'}, debug=False)
            elif targetcfg.language == 'python':
                analyzer.analyze_wrap({'anamode': 'listapisfrompy'}, debug=False)
            else:
